#include "stdafx.h"

DWORD Hypervisor::PeekPokeExpID = 0xD50F5D5A;
BYTE Hypervisor::FuseLines[0x10];

DWORD __declspec(naked) Hypervisor::GetVersions(DWORD Magic, DWORD Mode, QWORD Dest, QWORD Src, DWORD Len, QWORD Arg_r8) {
	__asm {
		li      r0, 0
		sc
		blr
	}
}

static QWORD ExpansionInstall(DWORD PhysicalAddress, DWORD CodeSize) {
	__asm {
		li			r0, 0x72
		sc
		blr
	}
}

static QWORD ExpansionCall(DWORD ExpansionId, QWORD Param1 = 0, QWORD Param2 = 0, QWORD Param3 = 0, QWORD Param4 = 0) {
	__asm {
		li			r0, 0x73
		sc
		blr
	}
}

BOOL Hypervisor::Initialize() {
	BYTE szPeekPoke[] = {
		
	};

	PeekPokeExpID ^= 0x9D590D0A;
	PVOID pPhysExp = XPhysicalAlloc(0x1000, MAXULONG_PTR, NULL, PAGE_READWRITE);
	DWORD physExpAdd = reinterpret_cast<DWORD>(MmGetPhysicalAddress(pPhysExp));

	ZeroMemory(pPhysExp, 0x1000);

	for (DWORD x = 0; x < sizeof(szPeekPoke); x++)
		szPeekPoke[x] ^= 0x95;
	memcpy(pPhysExp, szPeekPoke, sizeof(szPeekPoke));

	DWORD Result = static_cast<HRESULT>(ExpansionInstall(physExpAdd, 0x1000));
	XPhysicalFree(pPhysExp);
	return (SUCCEEDED(Result) ? TRUE : FALSE);
}

BYTE Hypervisor::PeekBYTE(QWORD Address) { return static_cast<BYTE>(ExpansionCall(PeekPokeExpID, PEEK_BYTE, Address)); }

WORD Hypervisor::PeekWORD(QWORD Address) { return static_cast<WORD>(ExpansionCall(PeekPokeExpID, PEEK_WORD, Address)); }

DWORD Hypervisor::PeekDWORD(QWORD Address) { return static_cast<DWORD>(ExpansionCall(PeekPokeExpID, PEEK_DWORD, Address)); }

QWORD Hypervisor::PeekQWORD(QWORD Address) { return ExpansionCall(PeekPokeExpID, PEEK_QWORD, Address); }

DWORD Hypervisor::PokeBYTE(QWORD Address, BYTE Value) { return static_cast<DWORD>(ExpansionCall(PeekPokeExpID, POKE_BYTE, Address, Value)); }

DWORD Hypervisor::PokeWORD(QWORD Address, WORD Value) { return static_cast<DWORD>(ExpansionCall(PeekPokeExpID, POKE_WORD, Address, Value)); }

DWORD Hypervisor::PokeDWORD(QWORD Address, DWORD Value) { return static_cast<DWORD>(ExpansionCall(PeekPokeExpID, POKE_DWORD, Address, Value)); }

DWORD Hypervisor::PokeQWORD(QWORD Address, QWORD Value) { return ExpansionCall(PeekPokeExpID, POKE_QWORD, Address, Value); }

DWORD Hypervisor::PeekBytes(QWORD Address, void* Buffer, DWORD Size) {
	void* data = XPhysicalAlloc(Size, MAXULONG_PTR, NULL, PAGE_READWRITE);
	ZeroMemory(data, Size);

	DWORD result = static_cast<DWORD>(ExpansionCall(PeekPokeExpID, PEEK_BYTES, Address, (QWORD)MmGetPhysicalAddress(data), Size));

	if (SUCCEEDED(result))
		memcpy(Buffer, data, Size);

	XPhysicalFree(data);
	return result;
}

DWORD Hypervisor::PokeBytes(QWORD Address, CONST void* Buffer, DWORD Size) {
	void* data = XPhysicalAlloc(Size, MAXULONG_PTR, NULL, PAGE_READWRITE);
	memcpy(data, Buffer, Size);

	DWORD result = static_cast<DWORD>(ExpansionCall(PeekPokeExpID, POKE_BYTES, Address, (QWORD)MmGetPhysicalAddress(data), Size));

	XPhysicalFree(data);
	return result;
}

QWORD Hypervisor::GetFuseLine(BYTE fuseIndex) {
	if (fuseIndex > 11) return 0;
	return PeekQWORD(0x8000020000020000 + (fuseIndex * 0x200));
}

BYTE* Hypervisor::GetHypervisorCPU() {
	BYTE cpukey[0x10];
	PeekBytes(0x20, cpukey, 0x10);
	return cpukey;
}

BYTE* Hypervisor::GetFuseCPU() {
	BYTE cpukey[0x10];
	UINT64 f1p1 = GetFuseLine(3);
	UINT64 f1p2 = GetFuseLine(5);
	memcpy(cpukey, &f1p1, 0x08);
	memcpy(cpukey + 0x08, &f1p2, 0x08);
	return cpukey;
}

BYTE* Hypervisor::GetConsoleType() {
	BYTE version[0x4];
	DWORD val = PeekDWORD(0x14);
	memcpy(version, &val, 0x4);
	return version;
}